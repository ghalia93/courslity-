// Handles API university review vote requests.
  ...
  const [rows] = await pool.query<VoteSummaryRow[]>(
    `SELECT
      ...
      (
        ...
        WHERE my_vote.university_review_id = ? AND my_vote.user_id = ?
        LIMIT 1
      ) AS user_vote
    FROM university_review_vote
    WHERE university_review_id = ?`,
...
export async function POST(
  ...
  try {
    ...
    const [reviews] = await pool.query<ReviewRow[]>(
      `SELECT university_review_id
       FROM university_review
       WHERE university_review_id = ?
    ...
    const [existingRows] = await pool.query<ExistingVoteRow[]>(
      "SELECT vote_value FROM university_review_vote WHERE university_review_id = ? AND user_id = ? LIMIT 1",
    ...
    if (existingVote === nextVote) {
      await pool.query<ResultSetHeader>(
        "DELETE FROM university_review_vote WHERE university_review_id = ? AND user_id = ?",
        [reviewId, user.userId],
      );
    } else if (existingRows.length > 0) {
      await pool.query<ResultSetHeader>(
        "UPDATE university_review_vote SET vote_value = ? WHERE university_review_id = ? AND user_id = ?",
        [nextVote, reviewId, user.userId],
      );
    } else {
      await pool.query<ResultSetHeader>(
        "INSERT INTO university_review_vote (university_review_id, user_id, vote_value) VALUES (?, ?, ?)",
    ...
    console.error("UNIVERSITY REVIEW VOTE ERROR:", error);
    return NextResponse.json(
